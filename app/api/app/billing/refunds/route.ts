...
export async function POST(request: Request) {
  ...
  if (!credentials.keyId || !credentials.keySecret) {
    return NextResponse.json(
      {
        error:
          'Live Razorpay credentials are not connected; no refund was created.',
      },
      { status: 409 },
    );
  }
  const payment = await getRawDb()
    .prepare(`SELECT id, amount FROM payment_reconciliations
    WHERE organization_id = ? AND external_id = ? AND provider = 'razorpay' AND status = 'matched'`)
    .bind(auth.session.organizationId, body.paymentId)
  ...
  const response = await fetch(
    `https://api.razorpay.com/v1/payments/${encodeURIComponent(body.paymentId)}/refund`,
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        amount,
        notes: {
          reason: body.reason?.slice(0, 200) || 'Customer-approved refund',
  ...
  if (!response.ok || !payload.id)
    return NextResponse.json(
      { error: payload.error?.description || 'Refund could not be created.' },
      { status: 502 },
    );
  await getRawDb()
  ...
  return NextResponse.json(
    { refundId: payload.id, status: payload.status },
    { status: 201 },
  );
}
