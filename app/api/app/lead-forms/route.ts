...
export async function POST(request: Request) {
  ...
  await getRawDb()
    ...
    .run();
  await recordAudit(auth.session, 'lead_form.created', 'lead_form', id, {
    publicKey,
  });
...
export async function PATCH(request: Request) {
  ...
  const result = await getRawDb()
    ...
    .run();
  await recordAudit(auth.session, `lead_form.${action}`, 'lead_form', body.id, {
    domains,
  });
  return NextResponse.json({
