...
export async function GET(request: Request) {
  ...
  const rows = await getRawDb()
    .prepare(`SELECT * FROM retargeting_audiences
...
export async function POST(request: Request) {
  ...
  if (body.action === 'create') {
    if (
      !body.name?.trim() ||
      !['meta_ads', 'google_ads'].includes(body.destination || '')
    )
      return NextResponse.json(
        { error: 'Name and supported destination are required.' },
        { status: 400 },
      );
    const eligible = await db
      .prepare(`SELECT count(*) AS count FROM leads l
    ...
    await db
      .prepare(`INSERT INTO retargeting_audiences
      ...
      .run();
    return NextResponse.json(
      { id, eligibleCount: Number(eligible?.count || 0) },
      { status: 201 },
    );
  }
  if (body.action === 'sync' && body.audienceId) {
    await db
      .prepare(
        `UPDATE retargeting_audiences SET status = 'syncing' WHERE id = ? AND organization_id = ?`,
      )
      .bind(body.audienceId, organizationId)
      .run();
    await enqueueJob({
      organizationId,
      queue: 'retargeting',
      type: 'retargeting.sync',
      idempotencyKey: `audience-sync:${body.audienceId}:${new Date().toISOString().slice(0, 13)}`,
      payload: { audienceId: body.audienceId },
    });
    return NextResponse.json({ status: 'syncing' }, { status: 202 });
  }
  return NextResponse.json(
    { error: 'Unsupported audience action.' },
    { status: 400 },
  );
}
