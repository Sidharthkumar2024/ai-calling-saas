...
export function CustomerCommerce({
  ...
  async function post(body: Record<string, unknown>) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(payload.error ?? 'Unable to run commerce action.');
      setNotice(
        payload.shortUrl
  ...
  const whatsapp = data.connections?.find(
    (item) => item.type === 'whatsapp_cloud',
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <Metric
          icon={CreditCard}
          label="Payment links"
          value={String(paymentLinks.length)}
          note="Razorpay-ready"
        />
        <Metric
          ...
          note="WhatsApp + email fallback"
        />
        <Metric
          icon={Clock3}
          label="Scheduled"
          value={String(
            actions.filter((item) => item.status === 'pending').length,
          )}
          note="Asia/Kolkata aware"
        />
        <Metric
          icon={IndianRupee}
          label="Payment intent"
          value={money(
            paymentLinks.reduce((sum, item) => sum + Number(item.amount), 0),
          )}
          note="Links created"
        />
      </div>
        ...
        <section className="rounded-2xl border border-hairline bg-surface p-5">
          ...
          <div className="mt-5 space-y-4">
              ...
              <div className="grid grid-cols-2 rounded-xl border border-hairline bg-surface-muted p-1">
                {(['instant', 'scheduled'] as const).map((item) => (
                  <button
                    ...
                    className={`rounded-lg px-3 py-2 text-[11px] capitalize ${form.deliveryMode === item ? 'bg-surface-strong text-ink' : 'text-ink-muted'}`}
                  >
            ...
            {form.deliveryMode === 'scheduled' ? (
              <Field label="Customer-requested time">
                <Input
                  type="datetime-local"
            ...
            >
              ...
              {form.deliveryMode === 'instant'
                ...
                : 'Create & schedule payment link'}
            </Button>
          </div>
          <div className="mt-5 grid gap-2 sm:grid-cols-3">
            <Connection
              label="Razorpay"
              value={razorpay?.status ?? 'local sandbox'}
            />
            <Connection
              label="WhatsApp"
              value={whatsapp?.status ?? 'local sandbox'}
            />
            <Connection
              label="Email fallback"
              value={email?.status ?? 'local sandbox'}
            />
          </div>
        </section>
          ...
          <div className="mt-5 space-y-2">
            {[
              [
                '1',
                'Understand intent',
                'Product details + ₹550 payment + requested delivery time',
              ],
              [
                '2',
                'Check policy',
                'Consent, destination, amount range and approved template',
              ],
              [
                '3',
                'Create link',
              ...
              [
                '4',
                'Schedule action',
                'Durable record with timezone and retry policy',
              ],
              [
                '5',
                'Send on WhatsApp',
                'Approved template with amount and secure link',
              ],
              [
                '6',
                'Close the loop',
                'Signed webhook updates payment, CRM and salesperson',
              ],
            ].map(([step, title, note]) => (
              <div
                key={step}
              ...
              >
                <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-violet-300/8 font-mono text-[11px] text-violet-700">
                  {step}
                </span>
                <div>
...
function Status({ value }: { value: string }) {
  const positive = ['paid', 'sent', 'sandbox_delivered', 'completed'].includes(
    value,
  );
  const warning = ['scheduled', 'created', 'processing', 'pending'].includes(
    value,
  );
  return (
    <span
