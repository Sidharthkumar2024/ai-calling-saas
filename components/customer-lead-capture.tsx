...
export function CustomerLeadCapture({
  ...
  const [settings, setSettings] = useState<LeadFormSettings | null>(
  ...
  async function createForm() {
    ...
    try {
      const response = await fetch('/api/app/lead-forms', {
        ...
        body: JSON.stringify({ name: 'Website callback popup' }),
      });
      ...
      setError(
        caught instanceof Error ? caught.message : 'Unable to create form.',
      );
    } finally {
      setSaving(false);
    }
  ...
  async function save(action: 'save' | 'publish' | 'unpublish') {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : 'Unable to save form.',
      );
    } finally {
      setSaving(false);
    }
          ...
          <div className="mt-5 space-y-4">
            ...
            <Field label="Placement">
              <div className="grid grid-cols-3 gap-2">
                {(['bottom_left', 'center_modal', 'bottom_right'] as const).map(
                  (placement) => (
                    <button
                      key={placement}
                      type="button"
            ...
            <div className="grid grid-cols-2 gap-3">
              <Select
                ...
                onChange={(value) =>
                  update('trigger', value as LeadFormSettings['trigger'])
                }
                options={[
                  ['delay', 'After delay'],
              ...
              <Field label="Delay seconds">
                <Input
                  ...
                  disabled={settings.trigger !== 'delay'}
                />
              </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Select
                ...
                onChange={(value) =>
                  update('animation', value as LeadFormSettings['animation'])
                }
                options={[
                  ['slide', 'Slide'],
                  ['fade', 'Fade'],
              ...
              <Select
                ...
                onChange={(value) =>
                  update('frequency', value as LeadFormSettings['frequency'])
                }
                options={[
                  ['every_visit', 'Every visit'],
                  ['once_session', 'Once / session'],
                  ['once_7_days', 'Once / 7 days'],
                ]}
              />
            </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            <section className="portal-panel p-5">
              ...
              <Button
                ...
                className="portal-primary mt-4 w-full"
              >
                <Settings2 /> Configure Meta & Google
              </Button>
            </section>
          </div>
        </div>
...
function Preview({
  ...
  const placement =
    settings.placement === 'bottom_left'
      ? 'bottom-5 left-5'
      : settings.placement === 'center_modal'
