...
function ResourceModule({ module, data, onChanged }: { module: Exclude<OperationsModule, 'call_history'|'live_monitor'|'analytics'|'quality'|'settings'>; data: OperationsData; onChanged: () => Promise<void> | void }) {
  ...
  return <div className="space-y-6"><Header eyebrow={config.eyebrow} title={config.title} description={config.description} action={<Button onClick={create} disabled={Boolean(loading)} className="bg-cyan-300 text-[#07101e] hover:bg-cyan-200">{loading === 'create' ? <Loader2 className="animate-spin" /> : <Plus />}{config.button}</Button>} />
...
function Analytics({ data }: { data: OperationsData }) {
  const outcomeCounts = useMemo(() => Object.entries(data.calls.reduce<Record<string, number>>((map, call) => { const key = str(call.outcome, 'unknown'); map[key] = (map[key] ?? 0) + 1; return map; }, {})), [data.calls]);
  const max = Math.max(1, ...outcomeCounts.map(([, value]) => value));
  return <div className="space-y-6"><Header eyebrow="Performance intelligence" title="Analytics" description="Call, outcome, latency, cost and conversion metrics generated from tenant-owned records." /><Stats data={data} /><div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]"><section className="rounded-2xl border border-white/8 bg-[#0c1422] p-5"><h2 className="text-sm font-semibold">Outcome distribution</h2><div className="mt-7 space-y-4">{outcomeCounts.map(([label, value]) => <div key={label}><div className="mb-2 flex justify-between text-[10px] text-white/42"><span>{label.replaceAll('_',' ')}</span><span>{value}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/6"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-violet-300" style={{ width: `${(value / max) * 100}%` }} /></div></div>)}</div></section><section className="rounded-2xl border border-white/8 bg-[#0c1422] p-5"><h2 className="text-sm font-semibold">Conversation trend</h2><div className="mt-7 flex h-56 items-end gap-3">{[38,54,49,68,61,76,88].map((value, index) => <div key={index} className="flex h-full flex-1 items-end"><div className="w-full rounded-t-lg bg-gradient-to-t from-cyan-300/20 to-cyan-300" style={{ height: `${value}%` }} /></div>)}</div><div className="mt-3 flex justify-between text-[9px] text-white/25"><span>Mon</span><span>Today</span></div></section></div></div>;
}
...
function Metric({ label, value, icon: Icon }: { label: string; value: string; icon: typeof Bot }) { return <section className="rounded-2xl border border-white/8 bg-[#0c1422] p-4"><Icon className="size-4 text-cyan-200" /><p className="mt-4 text-2xl font-semibold">{value}</p><p className="mt-1 text-[9px] uppercase tracking-wider text-white/28">{label}</p></section>; }
function Header({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: React.ReactNode }) { return <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-cyan-300/80">{eyebrow}</p><h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">{title}</h1><p className="mt-2 max-w-3xl text-xs leading-5 text-white/38">{description}</p></div>{action}</div>; }
