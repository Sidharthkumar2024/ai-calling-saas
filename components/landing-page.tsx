...
const industryStories = {
  'Real estate': {
    ...
    title: 'Qualify enquiries and book site visits while intent is still fresh.',
    points: [
  ...
  Education: {
    ...
    title: 'Follow up with applicants and keep counsellors focused on serious students.',
    points: [
  ...
  Commerce: {
    ...
    title: 'Recover demand, confirm orders and create repeatable sales motions.',
    points: [
    ...
    <main className="min-h-screen overflow-hidden bg-[#090b11] text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-white/8 bg-[#090b11]/88 backdrop-blur-xl">
        <div className="mx-auto flex h-[70px] max-w-[1240px] items-center justify-between px-4 sm:px-6">
          <a href="#top" className="flex items-center gap-3" aria-label="Vaani home">
            ...
            <span>
              <span className="block text-[15px] font-semibold tracking-tight">Vaani</span>
        ...
        <div className="relative mx-auto grid max-w-[1240px] gap-12 px-4 pb-20 pt-16 sm:px-6 sm:pt-24 xl:grid-cols-[minmax(0,0.95fr)_minmax(520px,1.05fr)] xl:items-center xl:gap-14 xl:pb-28">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <a href="#workflow" className="inline-flex h-12 items-center justify-center rounded-full border border-white/14 bg-white/4 px-6 text-sm font-medium text-white transition-colors hover:bg-white/8">
                Explore the workflow
              </a>
            </div>
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-[11px] text-white/45">
              {['India-ready languages', 'Consent & DNC controls', 'Admin + customer panels'].map((item) => (
              ...
              <div className="flex items-center justify-between border-b border-white/8 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-white/38">NCR buyer qualification</p>
                  </div>
                </div>
              ...
              <div className="grid lg:grid-cols-[1fr_185px]">
                <div className="min-w-0 border-b border-white/8 p-4 lg:border-b-0 lg:border-r">
                  <div className="mb-5 flex items-center gap-3 rounded-xl border border-white/8 bg-white/[0.025] p-3">
                    <div className="grid size-9 place-items-center rounded-full bg-cyan-300/12 text-xs font-semibold text-cyan-200">AM</div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-medium">Aditi Mehra</p>
                      <p className="truncate text-[10px] text-white/38">Meta Lead Ad · New enquiry</p>
                    </div>
                    <Badge className="border-0 bg-emerald-400/10 text-[9px] text-emerald-300">High intent</Badge>
                  </div>
                  ...
                  <div className="space-y-4 text-xs leading-5">
                    <div className="max-w-[88%] rounded-2xl rounded-tl-md bg-white/[0.055] p-3 text-white/72">
                      Hi Aditi, मैं Maya बोल रही हूँ UrbanNest से. आपने Dwarka Expressway project के बारे में enquiry की थी—क्या अभी दो मिनट हैं?
                    </div>
                    <div className="ml-auto max-w-[78%] rounded-2xl rounded-tr-md bg-amber-300 p-3 text-[#21190a]">
                      हाँ, 3 BHK देख रही हूँ. Budget करीब 2 crore है और possession जल्दी चाहिए.
                    </div>
                    <div className="max-w-[88%] rounded-2xl rounded-tl-md bg-white/[0.055] p-3 text-white/72">
                      Perfect. आपके budget में दो options match हो रहे हैं. मैं Saturday 11:30 का site visit reserve कर दूँ?
                    </div>
                  </div>

                  <div className="mt-6 flex h-9 items-end gap-1 rounded-xl border border-white/8 bg-black/20 px-3 py-2" aria-label="Live voice activity">
                    {[8, 13, 7, 18, 11, 22, 15, 10, 19, 8, 14, 6, 11, 17, 9, 5, 12, 7, 4].map((height, index) => (
                      <span key={`${height}-${index}`} className="landing-wave flex-1 rounded-full bg-violet-300/70" style={{ height }} />
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4 p-4">
                  <div>
                    <p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-white/35">Lead score</p>
                    <div className="mt-2 flex items-end gap-2">
                      <span className="font-mono text-3xl font-semibold text-emerald-300">92</span>
                      <span className="mb-1 text-[10px] text-white/35">/ 100</span>
                    </div>
                  </div>
                  ...
                  {[
                    ...
                    <div key={label} className="flex items-center justify-between border-b border-white/7 pb-2 text-[10px]">
                  ...
                  <div className="rounded-xl border border-emerald-400/15 bg-emerald-400/8 p-3">
                    ...
                    <p className="mt-1 text-[9px] leading-4 text-white/40">Saturday · 11:30 AM<br />CRM owner: Neha</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="border-y border-white/8 bg-white/[0.018]">
        <div className="mx-auto grid max-w-[1240px] grid-cols-2 divide-x divide-y divide-white/8 px-4 sm:grid-cols-4 sm:px-6 lg:grid-cols-7 lg:divide-y-0">
          {['Meta Lead Ads', 'Google Ads', 'Website forms', 'CRM', 'WhatsApp', 'API & webhooks', 'Google Sheets'].map((source) => (
            <div key={source} className="flex h-20 items-center justify-center gap-2 px-3 text-center text-[11px] font-medium text-white/48">
              <span className="size-1.5 rounded-full bg-amber-300/70" /> {source}
            </div>
          ))}
        </div>
      </section>

      <section id="workflow" className="landing-lazy scroll-mt-24 border-b border-white/8 py-24 sm:py-28">
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="grid gap-8 lg:grid-cols-[0.75fr_1.25fr] lg:items-end">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-300">From click to close</p>
              <h2 className="mt-4 max-w-lg text-4xl font-semibold leading-[1.05] tracking-[-0.045em] sm:text-5xl">One continuous revenue loop.</h2>
            </div>
          ...
          <div className="mt-14 grid overflow-hidden rounded-2xl border border-white/10 bg-white/[0.018] md:grid-cols-2 xl:grid-cols-4">
            {revenueLoop.map((item, index) => (
              <div key={item.step} className="relative border-b border-white/8 p-6 last:border-b-0 md:[&:nth-child(odd)]:border-r xl:border-b-0 xl:border-r xl:last:border-r-0">
                ...
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[10px] text-white/28">{item.step}</span>
      ...
      <section id="product" className="landing-grid landing-lazy scroll-mt-24 py-24 sm:py-28">
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-300">Complete operating system</p>
            <h2 className="mt-4 text-4xl font-semibold tracking-[-0.045em] sm:text-5xl">Everything around the conversation.</h2>
            <p className="mt-5 text-sm leading-6 text-white/48">A focused customer workspace and a powerful platform admin—without raw infrastructure leaking into the product.</p>
          </div>

          <div className="mt-14 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {capabilities.map((item, index) => (
              <article key={item.title} className={`group min-h-[230px] rounded-2xl border border-white/9 bg-[#10131b] p-5 transition-colors hover:border-white/18 hover:bg-[#131722] ${index === 0 || index === 7 ? 'sm:col-span-2' : ''}`}>
                <div className="flex items-start justify-between">
                  <span className="grid size-10 place-items-center rounded-xl border border-white/8 bg-white/[0.035] text-amber-300"><item.icon className="size-4.5" /></span>
                  <span className="font-mono text-[9px] uppercase tracking-[0.16em] text-white/28">{item.meta}</span>
                </div>
                <h3 className="mt-10 text-lg font-medium tracking-tight">{item.title}</h3>
                <p className="mt-3 max-w-md text-xs leading-5 text-white/42">{item.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="solutions" className="landing-lazy scroll-mt-24 border-y border-white/8 bg-[#0d1017] py-24 sm:py-28">
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="grid gap-10 lg:grid-cols-[310px_1fr]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-300">Built for outcomes</p>
              <h2 className="mt-4 text-4xl font-semibold tracking-[-0.045em]">One platform.<br />Different revenue plays.</h2>
              <div className="mt-8 space-y-2" role="tablist" aria-label="Industry solutions">
                {(Object.keys(industryStories) as Array<keyof typeof industryStories>).map((name) => (
                  <button key={name} type="button" role="tab" aria-selected={activeUseCase === name} onClick={() => setActiveUseCase(name)} className={`flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-sm transition-colors ${activeUseCase === name ? 'bg-white text-black' : 'text-white/48 hover:bg-white/5 hover:text-white'}`}>
                    {name}<ArrowRight className="size-3.5" />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-[24px] border border-white/10 bg-[#11141d] p-6 sm:p-8 lg:p-10">
              <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-xl">
                  <div className="grid size-12 place-items-center rounded-2xl bg-emerald-300/10 text-emerald-300"><CurrentStoryIcon className="size-5" /></div>
                  <p className="mt-8 text-xs font-semibold uppercase tracking-[0.16em] text-emerald-300">{currentStory.eyebrow}</p>
                  <h3 className="mt-3 text-2xl font-medium leading-tight tracking-[-0.03em] sm:text-3xl">{currentStory.title}</h3>
                  <ul className="mt-7 space-y-3">
                    {currentStory.points.map((point) => (
                      <li key={point} className="flex gap-3 text-sm leading-6 text-white/52"><CheckCircle2 className="mt-1 size-4 shrink-0 text-emerald-300" />{point}</li>
                    ))}
                  </ul>
                </div>
                <div className="w-full max-w-[255px] rounded-2xl border border-white/9 bg-black/25 p-5">
                  <p className="text-[9px] font-semibold uppercase tracking-[0.18em] text-white/30">Live outcome</p>
                  <p className="mt-5 text-lg font-medium leading-7">{currentStory.outcome}</p>
                  <div className="mt-6 space-y-3">
                    {['Lead understood', 'Call completed', 'CRM updated', 'Next action queued'].map((label, index) => (
                      <div key={label} className="flex items-center gap-2 text-[10px] text-white/45"><span className={`size-1.5 rounded-full ${index < 3 ? 'bg-emerald-400' : 'bg-amber-300'}`} />{label}</div>
                    ))}
                  </div>
                </div>
              </div>
      ...
      <section id="engines" className="landing-lazy scroll-mt-24 py-24 sm:py-28">
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300">The Vaani engine family</p>
              <h2 className="mt-4 text-4xl font-semibold leading-[1.05] tracking-[-0.045em] sm:text-5xl">One brand across the entire voice stack.</h2>
            </div>
            <p className="max-w-xl text-sm leading-6 text-white/48 lg:justify-self-end">Customers work with Vaani’s product capabilities—not a maze of infrastructure vendors, model IDs or provider credentials.</p>
          </div>
          <div className="mt-14 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {VAANI_ENGINES.map((engine) => (
              <article key={engine.name} className="rounded-2xl border border-white/9 bg-white/[0.022] p-5">
                <div className="flex items-center gap-3">
                  <span className="grid size-9 place-items-center rounded-xl bg-cyan-300/10 text-cyan-300"><engine.icon className="size-4" /></span>
                  <div><h3 className="text-sm font-medium">{engine.name}</h3><p className="mt-0.5 text-[9px] uppercase tracking-[0.14em] text-white/28">{engine.role}</p></div>
                </div>
                <p className="mt-6 text-xs leading-5 text-white/42">{engine.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="security" className="landing-lazy border-y border-white/8 bg-[#0d1017] py-20 sm:py-24">
        <div className="mx-auto grid max-w-[1240px] gap-10 px-4 sm:px-6 lg:grid-cols-[1fr_1fr] lg:items-center">
          <div>
            <Badge variant="outline" className="gap-2 rounded-full border-emerald-400/18 bg-emerald-400/7 text-emerald-300"><ShieldCheck className="size-3.5" /> Built for responsible calling</Badge>
            <h2 className="mt-6 text-4xl font-semibold tracking-[-0.045em]">Strong controls for teams that call at scale.</h2>
            <p className="mt-5 max-w-xl text-sm leading-6 text-white/48">Tenant isolation, role-based access, consent evidence, DNC suppression, audit trails and human escalation are part of the operating model—not afterthoughts.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              [LockKeyhole, 'Tenant isolation', 'Every company’s leads, knowledge and calls stay separated.'],
              [UsersRound, 'Role-based access', 'Customer, agent and platform-admin permissions stay distinct.'],
              [BadgeCheck, 'Consent evidence', 'Source, timestamp and calling permission travel with each lead.'],
              [Webhook, 'Auditable automation', 'Every workflow run and external delivery has a traceable status.'],
            ].map(([Icon, title, description]) => {
              const SecurityIcon = Icon as typeof LockKeyhole;
              return <div key={title as string} className="rounded-2xl border border-white/8 bg-white/[0.02] p-5"><SecurityIcon className="size-4 text-emerald-300" /><h3 className="mt-6 text-sm font-medium">{title as string}</h3><p className="mt-2 text-xs leading-5 text-white/38">{description as string}</p></div>;
            })}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_85%_10%,rgba(167,139,250,0.19),transparent_28%),radial-gradient(circle_at_10%_90%,rgba(252,211,77,0.14),transparent_30%),#12151e] px-6 py-12 sm:px-10 lg:flex lg:items-end lg:justify-between lg:px-14 lg:py-16">
            <div className="max-w-2xl">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-300">Your next lead is already waiting</p>
              <h2 className="mt-5 text-4xl font-semibold leading-tight tracking-[-0.045em] sm:text-5xl">Give every enquiry a real conversation.</h2>
              <p className="mt-5 text-sm leading-6 text-white/48">Open the product workspace to explore customer and admin panels, CRM, lead capture, calling and retargeting.</p>
            </div>
            <Button onClick={onEnterWorkspace} size="lg" className="mt-8 h-12 rounded-full bg-white px-6 text-black hover:bg-white/90 lg:mt-0">Open Vaani platform <ArrowRight /></Button>
          </div>
        </div>
      </section>
