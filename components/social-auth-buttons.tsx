...
export function SocialAuthButtons() {
  ...
  return <div className="mt-5 space-y-2">{providers.map((provider) => {
    ...
    return <button key={provider.provider} type="button" disabled={!ready} title={ready ? `Continue with ${provider.display_name}` : 'Activation is controlled from the admin portal'} className="flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-white/[0.035] text-xs font-medium text-white/78 transition-colors enabled:hover:bg-white/[0.07] disabled:cursor-not-allowed disabled:text-white/40"><span className="grid size-5 place-items-center rounded-full bg-white text-[11px] font-bold text-[#3157c8]">G</span>Continue with {provider.display_name}{!ready ? <span className="rounded-full bg-amber-300/10 px-2 py-1 text-[8px] font-normal text-amber-200">Admin disabled</span> : null}</button>;
