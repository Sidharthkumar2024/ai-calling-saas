...
export async function settleExotel(db: D1Database, org: string, callId: string, credits: number) {
  if (!Number.isSafeInteger(credits) || credits <= 0) throw new Error('Invalid call charge.');
  const ledgerId = `exotel_settlement_${callId}`;
  if (await db.prepare('SELECT id FROM credit_ledger WHERE id = ?').bind(ledgerId).first()) return false;
  const wallet = await db.prepare('SELECT organization_id FROM organization_wallets WHERE organization_id = ?').bind(org).first();
  if (!wallet) throw new Error('Call wallet is missing; settlement requires review.');
  try {
    await db.batch([
      db.prepare(EXOTEL_SETTLEMENT_SQL[0]).bind(ledgerId, -credits, credits, callId, org),
      db.prepare(EXOTEL_SETTLEMENT_SQL[1]).bind(credits, org),
      db.prepare(EXOTEL_SETTLEMENT_SQL[2]).bind(credits, callId, org),
    ]);
    return true;
  } catch (error) {
    if (!await db.prepare('SELECT id FROM credit_ledger WHERE id = ?').bind(ledgerId).first()) throw error;
    return false;
  }
}
