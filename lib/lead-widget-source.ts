...
// A field is not always an <input>: a dropdown whose choices are the accepted
// answers, a checkbox that must sit beside its own words, a box for a
// paragraph. Rendered from the same field list the endpoint validates against.
const control=f=>{
if(f.type==='select')return '<select name="'+esc(f.key)+'" '+(f.required?'required':'')+'><option value="">Choose\u2026</option>'+(f.options||[]).map(o=>'<option>'+esc(o)+'</option>').join('')+'</select>';
if(f.type==='textarea')return '<textarea name="'+esc(f.key)+'" rows="3" maxlength="5000" '+(f.required?'required':'')+'></textarea>';
if(f.type==='checkbox')return '<input class="tick" name="'+esc(f.key)+'" type="checkbox" value="yes" '+(f.required?'required':'')+'>';
return '<input name="'+esc(f.key)+'" type="'+esc(f.type)+'" maxlength="1000" '+(f.required?'required':'')+'>';
};
const row=f=>f.type==='checkbox'?'<label class="tickrow">'+control(f)+'<span>'+esc(f.label)+(f.required?' *':'')+'</span></label>':'<label>'+esc(f.label)+(f.required?' *':'')+control(f)+'</label>';
...
root.innerHTML='<style>:host{all:initial}.wrap{position:fixed;z-index:2147483000;'+position+'}.card{box-sizing:border-box;width:min(380px,calc(100vw - 28px));max-height:calc(100dvh - 40px);overflow:auto;padding:24px;border:1px solid #80808040;border-radius:24px;background:'+bg+';color:'+ink(bg)+';box-shadow:0 20px 70px #0003;font:14px/1.5 system-ui,sans-serif;animation:enter .3s ease-out}.close{float:right;border:0;background:none;color:inherit;font:22px system-ui;cursor:pointer}.logo{max-width:140px;height:40px;object-fit:contain}h2{margin:8px 24px 8px 0;font-size:23px;line-height:1.2}p{opacity:.8}form{display:grid;gap:12px}label{display:grid;gap:5px;font-size:13px}input,select,textarea{box-sizing:border-box;width:100%;min-height:44px;border:1px solid #8888;background:transparent;color:inherit;border-radius:10px;padding:10px;font:inherit}select option{color:#101b15}textarea{resize:vertical}.tickrow{display:flex;align-items:center;gap:9px}.tick{width:19px;min-height:19px;flex:0 0 auto;accent-color:'+accent+'}input:focus-visible,select:focus-visible,textarea:focus-visible,.send:focus-visible,.close:focus-visible{outline:2px solid '+accent+';outline-offset:3px}.send{border:0;border-radius:12px;min-height:46px;background:'+accent+';color:'+ink(accent)+';font:600 14px system-ui;cursor:pointer}.send:disabled{opacity:.5}.note{font-size:12px;margin:0}.result{min-height:20px;font-size:13px}@keyframes enter{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@media(prefers-reduced-motion:reduce){.card{animation:none}}</style><div class="wrap"><section class="card" aria-label="Callback request"><button class="close" aria-label="Close form">×</button>'+(s.logoUrl?'<img class="logo" alt="Business logo" referrerpolicy="no-referrer" src="'+esc(s.logoUrl)+'">':'')+'<h2>'+esc(s.title||'Let us call you back')+'</h2><p>'+esc(s.description||'Share your enquiry with our team.')+'</p><form>'+c.fields.map(row).join('')+'<p class="note">By requesting a callback, you ask this business to contact you about this enquiry.</p><button class="send" type="submit">'+esc(s.buttonText||'Request a call')+'</button><div class="result" role="status"></div></form></section></div>';
...
// The form is held on to before the first await. Inside a shadow root an
// event's target reads as null once dispatch has finished, so reading it after
// the fetch threw on every successful send \u2014 the lead was captured and the
// visitor was shown "Cannot read properties of null" over a form that never
// cleared, so anyone who tried again sent it twice.
root.querySelector('form').onsubmit=async e=>{e.preventDefault();const form=e.target,button=root.querySelector('.send'),status=root.querySelector('.result');button.disabled=true;status.textContent='Sending…';try{const body=Object.fromEntries(new FormData(form));body.pageUrl=location.origin+location.pathname;body.campaignName=document.title;const response=await fetch(c.endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const result=await response.json();if(!response.ok)throw Error(result.error||'Could not send your request.');status.textContent=s.successMessage||'Thanks — your request was received.';form.reset()}catch(error){status.textContent=error.message||'Please try again.'}finally{button.disabled=false}};
}
