...
const provider = String(process.argv[productionMode ? 3 : 2] ?? "")
  .trim()
  .toLowerCase();
const databasePath =
...
if (
  ...
  console.error(
    "Usage: CALLVANI_SQLITE_PATH=/absolute/app.sqlite VAANI_ENCRYPTION_KEY=... node scripts/store-platform-provider-secret.mjs [--production] <deepgram|cartesia|elevenlabs|sarvam>",
  );
  process.exit(2);
}
...
function readSecretFile(path) {
  ...
  if ((mode & 0o077) !== 0)
    throw new Error(
      "Provider secret file must be readable only by its owner (chmod 600).",
    );
  try {
...
const secretFile = process.env.CALLVANI_PROVIDER_SECRET_FILE?.trim();
if (!secretFile && (!process.stdin.isTTY || !process.stdout.isTTY)) {
  console.error(
...
const apiKey = (
  secretFile
    ...
    : await hidden(`${provider} API key (hidden): `)
).trim();
