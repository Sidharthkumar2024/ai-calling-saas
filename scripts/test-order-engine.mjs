...
const priced = priceOrder([
  line({ quantity: 2 }),
  line({ recordId: 'r2', title: 'Mat', unitPrice: 50000 }),
]);
ok('lines are priced and totalled', priced.ok && priced.subtotal === 250000);
ok(
  'each line carries its own total',
  priced.ok && priced.lines[0].lineTotal === 200000,
);
ok('an empty order is refused', priceOrder([]).ok === false);
ok(
  'quantity below one is refused',
  priceOrder([line({ quantity: 0 })]).ok === false,
);
ok(
  'a fractional quantity truncates, it does not round up',
  priceOrder([line({ quantity: 2.9 })]).lines[0].quantity === 2,
);
ok(
  'an absurd quantity is refused',
  priceOrder([line({ quantity: 100000 })]).ok === false,
);
ok(
  'a negative price is refused',
  priceOrder([line({ unitPrice: -1 })]).ok === false,
);
...
ok(
  'exactly the last unit is allowed',
  priceOrder([line({ quantity: 2, available: 2 })]).ok === true,
);
ok(
  'an item that does not track stock is unrestricted',
  priceOrder([line({ quantity: 900, available: null })]).ok === true,
);
...
const withAsset = priceOrder([
  line({ kind: 'digital', deliveryAsset: 'https://cdn.example.com/a.pdf' }),
]);
ok('with a file attached it prices normally', withAsset.ok === true);
ok(
  'the order knows it contains something digital',
  withAsset.hasDigital === true,
);
...
ok(
  ...
  pending.allowed === false &&
    pending.reason === 'awaiting_payment' &&
    pending.status === 402,
);
ok(
...
ok(
  'a revoked entitlement is gone for good',
  entitlementAccess({ ...base, status: 'revoked' }).reason === 'revoked',
);
ok(
  'an expired link is refused',
  entitlementAccess({ ...base, expires_at: '2020-01-01T00:00:00.000Z' })
    .reason === 'expired',
);
ok(
  'a link expiring in the future still opens',
  entitlementAccess({ ...base, expires_at: '2999-01-01T00:00:00.000Z' })
    .allowed === true,
);
ok(
...
ok(
  'no cap means no cap',
  entitlementAccess({ ...base, download_count: 9999, max_downloads: null })
    .allowed === true,
);
ok(
...
ok(
  'an unpaid order is never described as confirmed',
  !orderSayToCustomer('awaiting_payment', true).includes('confirmed and'),
);
ok(
  'an unpaid digital order promises the unlock, not the file',
  orderSayToCustomer('awaiting_payment', true).includes('unlocks'),
);
ok(
  'a paid digital order says the link is active',
  orderSayToCustomer('paid', true).includes('now active'),
);
ok(
  'statuses are validated',
  isOrderStatus('paid') && !isOrderStatus('probably_paid'),
);
