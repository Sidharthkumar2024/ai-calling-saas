...
check('an allowed type of a sane size is accepted', () => {
  const result = checkIncomingMedia({ mimeType: 'image/jpeg', sizeBytes: 200_000, bytes: jpeg });
...
check('a type nobody allowed is refused by name', () => {
  const result = checkIncomingMedia({ mimeType: 'application/x-msdownload', sizeBytes: 1000 });
...
check('a mime type with a charset suffix still matches', () => {
  assert.equal(checkIncomingMedia({ mimeType: 'application/pdf; charset=binary', sizeBytes: 500, bytes: pdf }).ok, true);
});
...
check('an executable renamed as a JPEG is caught by its bytes', () => {
  ...
  const result = checkIncomingMedia({ mimeType: 'image/jpeg', sizeBytes: 4000, bytes: exe });
  assert.equal(result.ok, false);
...
check('a PNG announced as a PDF is caught too', () => {
  assert.equal(
    checkIncomingMedia({ mimeType: 'application/pdf', sizeBytes: 4000, bytes: png }).reason,
    'content_mismatch',
  );
});

check('the right bytes for the declared type pass', () => {
  assert.equal(checkIncomingMedia({ mimeType: 'image/png', sizeBytes: 900, bytes: png }).ok, true);
  assert.equal(checkIncomingMedia({ mimeType: 'application/pdf', sizeBytes: 900, bytes: pdf }).ok, true);
});
...
check('mp4 is matched at its offset, not at byte zero', () => {
  ...
  assert.equal(checkIncomingMedia({ mimeType: 'video/mp4', sizeBytes: 5000, bytes: mp4 }).ok, true);
});

check('a truncated file does not pass by being too short to disprove', () => {
  assert.equal(
    checkIncomingMedia({ mimeType: 'image/png', sizeBytes: 3, bytes: new Uint8Array([0x89]) }).reason,
    'content_mismatch',
  );
});
...
check('over the per-kind cap is refused with both numbers', () => {
  const result = checkIncomingMedia({ mimeType: 'image/jpeg', sizeBytes: MAX_BYTES.image + 1 });
...
check('a document may be larger than an image', () => {
  ...
  assert.equal(checkIncomingMedia({ mimeType: 'application/pdf', sizeBytes: MAX_BYTES.image + 1, bytes: pdf }).ok, true);
});
...
check('an unstated size is refused rather than fetched to find out', () => {
  ...
  assert.equal(checkIncomingMedia({ mimeType: 'image/jpeg', sizeBytes: 0 }).reason, 'unknown_size');
  assert.equal(checkIncomingMedia({ mimeType: 'image/jpeg', sizeBytes: NaN }).reason, 'unknown_size');
});

check('checks run before bytes are available, so the size check works alone', () => {
  assert.equal(checkIncomingMedia({ mimeType: 'image/jpeg', sizeBytes: 1000 }).ok, true);
});
...
check('the allowlist has no archives or executables in it', () => {
  for (const mime of Object.keys(ALLOWED_INCOMING))
    assert.doesNotMatch(mime, /zip|rar|7z|msdownload|octet-stream|javascript|html/);
});

console.log('\ndocument states');

check('every status is a known status', () => {
  for (const status of DOCUMENT_STATUSES) assert.equal(isDocumentStatus(status), true);
  assert.equal(isDocumentStatus('approved'), false);
});

check('a new document can go anywhere a reviewer needs', () => {
  assert.deepEqual(nextStatuses('new'), ['under_review', 'accepted', 'rejected', 'needs_new_file']);
});
...
const asset = (id, kind = 'image', extra = {}) => ({
  id, kind, label: `${id}.${kind}`, url: `https://x.test/${id}`, ...extra,
});
...
check('a sensitive file is held for a person, not quietly included', () => {
  const set = buildSendSet({ requested: [asset('a'), asset('secret', 'document', { sensitive: true })] });
...
check('beyond the per-message limit is held back, not truncated silently', () => {
  const set = buildSendSet({
...
check('a partial send tells the agent NOT to claim it all arrived', () => {
  ...
  const set = buildSendSet({ requested: [asset('a'), asset('s', 'document', { sensitive: true })] });
